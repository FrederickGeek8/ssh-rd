...
int itmd_start_mux_tunnel(int localPort, int remotePort) {
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (const char*)&temp,
                   sizeof(temp))) {
    ...
    lpThreadId = pthread_create(&socket_thread, NULL, wait_for_device,
                                (void*)(size_t)remotePort);
...
char* getConnectedDeviceName(
    ...
    *deviceName = '\0';
    if (devId != nil) {
        CFStringGetCString(devId, deviceName, sizeof(deviceName),
...
int my_AMDeviceGetInterfaceType(am_device_t am_device) {
    ...
    if (pfn_AMDeviceGetInterfaceType == NULL) {
        ...
        if (hmItmd != NULL) {
            pfn_AMDeviceGetInterfaceType =
                (pfn_AMDeviceGetInterfaceType_t)GetProcAddress(
                    hmItmd, "AMDeviceGetInterfaceType");
        }
    }
    if (pfn_AMDeviceGetInterfaceType != NULL) {
...
void mux_notification_callback(
    struct am_device_notification_callback_info* info, void* context) {
    ...
    while (1) {
        ...
        if (muxConn == 0) {
            ...
            if (ret == ERR_SUCCESS) {
                ...
                muxConn = mux_tmp;
                //                restore_dev = AMRestoreModeDeviceCreate(0,
                //                mux_tmp, 0); fprintf(stderr, "restore_dev =
                //                %p\n", restore_dev); if (restore_dev != NULL)
                //                {
                //                    AMRestoreModeDeviceReboot(restore_dev);
                //                    Sleep(5 * 1000);
                //                }
            } else if (ret == -402653083) {  // after we call 'reboot', api host
                                             // is down
                muxconn_t mux_tmp = AMDeviceGetConnectionID(s_target_device);
                fprintf(stderr, "muxConnTmp = %X\n", mux_tmp);
                muxConn = mux_tmp;
        ...
        fprintf(stderr, "USBMuxConnectByPort OK\n");
        {
            ...
            lpThreadId = pthread_create(&thread1, NULL, conn_forwarding_thread,
                                        (void*)connection1);
            lpThreadId2 = pthread_create(&thread2, NULL, conn_forwarding_thread,
                                         (void*)connection2);
...
void* THREADPROCATTR conn_forwarding_thread(void* arg) {
    ...
    while (1) {
        ...
        if (bytes_recv == 0 || bytes_recv == SOCKET_ERROR || bytes_send == 0 ||

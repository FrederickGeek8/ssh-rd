...
public class Background implements Runnable {
	...
	Hashtable<String, String> filePropsByName(String name) {
		...
		if (!ios5) {
			norPatch = (device.isWtf() && ios3) ? wtfPatch
					: device.isArmV6() ? "nor_armv6.patch.json" : "nor.patch.json";
			kernelPatch = device.isArmV6() ? (ios3 ? "kernel3.patch.json" : "kernel_armv6.patch.json")
	...
	static boolean getFileFromZip(String zipUrl, String zipPath, String downloadPath) {
		...
		while (!zipFile.exists()) {
			gui.log(gui.MessageStyle.Important, "Please put %s in the %s directory (URL not public)", zipName,
					workingDir());
			try {
				Thread.sleep(5 * 1000);
	...
	void prepareRamdiskForDevice() {
		...
		String manifestPlistFile = downloadAndProcessFile("BuildManifesto.plist");
		if (manifestPlistFile == null) {
			gui.error("BuildManifesto.plist download failed!");
			return;
		}
		gui.trace("BuildManifesto.plist downloaded to %s", manifestPlistFile);
		...
		if (!device.isWtfStub()) {
			...
			String deviceTreePath = String.format("Firmware/all_flash/all_flash.%s.production/%s", device.getAp(),
					deviceTreeName);

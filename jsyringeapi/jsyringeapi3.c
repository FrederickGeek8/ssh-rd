...
Java_Jsyringe_fuzzy_1patch(JNIEnv* env, jclass jClass, jstring Jorig,
                           jstring Jpatched, jstring Jdelta, jint fuzz) {
...
static jboolean itmd_ensure() {
    ...
    int result;
    if (s_itmd_loaded) return JNI_TRUE;
 ...
 */
JNIEXPORT jboolean JNICALL Java_Jsyringe_restore_1bundle(JNIEnv* env,
                                                         jclass jClass,
                                                         jstring JbundlePath) {
    jboolean jresult = JNI_FALSE;
    const char* bundlePath;
    J2C(bundlePath);
    if (!itmd_ensure()) goto cleanup;
...
void javaMobileDeviceCallbackProc(void* ctx, int eventType, int productId,
                                  int productType) {
    PJAVA_CALLBACK_CONTEXT pctx = (PJAVA_CALLBACK_CONTEXT)ctx;

    jclass mobileDeviceClass =
        (*pctx->env)->GetObjectClass(pctx->env, pctx->jObject);
    // const char* callbackSignature =
    // "(ILjava/lang/String;Ljava/lang/String;)V";
    const char* callbackSignature = "(III)V";
    jmethodID mid = (*pctx->env)
                        ->GetMethodID(pctx->env, mobileDeviceClass, "callback",
                                      callbackSignature);
    if (mid == 0) {
        goto cleanup;
    }
    (*pctx->env)
        ->CallVoidMethod(pctx->env, pctx->jObject, mid, eventType, productId,
                         productType);
cleanup:
 ...
 */
JNIEXPORT void JNICALL Java_Jsyringe_runMobileDeviceThread(JNIEnv* env,
                                                           jclass jClass,
                                                           jobject jObject) {
    PJAVA_CALLBACK_CONTEXT pctx =
        (PJAVA_CALLBACK_CONTEXT)malloc(sizeof(JAVA_CALLBACK_CONTEXT));
    pctx->env = env;
    pctx->jClass = jClass;
 ...
 * Signature: (II)Z
 */
JNIEXPORT jboolean JNICALL Java_Jsyringe_startMuxThread(JNIEnv* env,
                                                        jclass jClass,
                                                        jint iport,
                                                        jint lport) {
    jboolean jresult = JNI_FALSE;
    ...
    if (0 == itmd_start_mux_tunnel(lport, iport)) jresult = JNI_TRUE;

...
JNIEXPORT jint JNICALL Java_Jsyringe_download_1file_1from_1zip(
    JNIEnv *env, jclass jClassObj, jstring Jurl, jstring Jpath,
    jstring Joutput) {
    jint result = -1;
    ...
    result = download_file_from_zip(url, path, output,
                                    NULL /*no callback just yet*/);
 ...
 */
JNIEXPORT jboolean JNICALL Java_Jsyringe_wait_1for_1connect(JNIEnv *env,
                                                            jclass jClass) {
    jboolean jresult = JNI_FALSE;
    if (g_syringe_client != NULL) {
        irecv_close(&g_syringe_client);
    }
    if (0 == irecv_open(&g_syringe_client) &&
        g_syringe_client->mode == kDfuMode) {
        if (irecv_get_device(g_syringe_client, &g_syringe_device) ==
            IRECV_E_SUCCESS) {
...
cleanup:
    if (g_syringe_client != NULL) irecv_close(&g_syringe_client);
    return jresult;
}
...
__declspec(dllexport) int tethered_boot(const char *ibssFile,
                                        const char *ibecFile,
                                        const char *kernelcacheFile,
                                        const char *ramdiskFile,
                                        const char *devicetreeFile);

int tethered_boot(const char *ibssFile, const char *ibecFile,
                  const char *kernelcacheFile, const char *ramdiskFile,
                  const char *devicetreeFile) {
    int result = 0;
...
//  (JNIEnv * env, jclass jClass, jstring JibssFile, jstring JibecFile, jstring
//  JkernelcacheFile, jstring JramdiskFile, jstring JdevicetreeFile)
//{
//	const char *ibssFile, *ibecFile, *kernelcacheFile, *ramdiskFile,
//*devicetreeFile; 	int result = 0;
//
//	J2C(ibssFile);
//	J2C(ibecFile);
//	J2C(kernelcacheFile);
...
//	J2C(devicetreeFile);
//
//	result = tethered_boot(ibssFile, ibecFile, kernelcacheFile, ramdiskFile,
// devicetreeFile);
//
//	JFREE(ibssFile);
//	JFREE(ibecFile);
 ...
 */
JNIEXPORT jstring JNICALL Java_Jsyringe_get_1device_1model(JNIEnv *env,
                                                           jclass jClass) {
...
BOOL WINAPI DllMain(__in HINSTANCE hinstDLL, __in DWORD fdwReason,
 ...
 * Signature: ()I
 */
JNIEXPORT jint JNICALL Java_Jsyringe_exploit(JNIEnv *env, jclass jClass) {
    int result = 0;
    irecv_error_t ir_error = IRECV_E_SUCCESS;
    irecv_client_t client = g_syringe_client;
